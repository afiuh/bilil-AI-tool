...
1. **精校文本**：加标点、分段、修正明显转写错误（如"沈沛→审配"）。
   分段规则：
   - 每段精校文本控制在80~150字（3~5句话），禁止超过200字的大段
   - 遇到逻辑转折、话题切换、举例/论证切换时必须分段
   - 每段配一个概括性的段落标题（不超过10字）
   - 对话/引语单独成段
...
要求：
- 覆盖全部原文，不要遗漏任何内容
- 每段精校文本 80~150 字，超过 200 字立即拆分
- 段落标题 ≤10 字，概括该段核心观点
- 逻辑转折处必须分段，禁止两个不同论点挤在同一段
- 如果原文有逻辑不通处，微调使其通顺
"""
